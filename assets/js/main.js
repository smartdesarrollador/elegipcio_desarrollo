$(document).ready(function () {
  // scroll para el menu de navegacion donde se muestra el logo grande y el logo pequeño dependiendo del scroll de la pagina
...
// Ingresar solo numeros en los input
...
let btnCloseAddressSelectorModal = D.getElementById(
  "btnCloseAddressSelectorModal"
);
...
let selectStoreSelectorLince = D.getElementById("selectStoreSelectorLince");
let selectStoreSelectorSurco = D.getElementById("selectStoreSelectorSurco");
let selectStoreSelectorSanMiguel = D.getElementById(
...
selectStoreSelectorLince.addEventListener("click", (ev) => {
  selectStoreSelectorElement.value =
    selectStoreSelectorLince.getAttribute("value");
...
selectStoreSelectorSurco.addEventListener("click", (ev) => {
  selectStoreSelectorElement.value =
    selectStoreSelectorSurco.getAttribute("value");
...
selectStoreSelectorSanMiguel.addEventListener("click", (ev) => {
  selectStoreSelectorElement.value =
    selectStoreSelectorSanMiguel.getAttribute("value");
...
formStoreSelector.addEventListener("submit", async function (event) {
  ...
  let shippingTypeInput = Array.from(this.elements.tipoReparto).find(
    (radio) => radio.checked
  );
  ...
  for (let i = 0; i < coordenadasDB.length; i++) {
    const polyObject = {
      polygon: new google.maps.Polygon({
        paths: JSON.parse(coordenadasDB[i].coords),
      }),
      ...
      if (
        google.maps.geometry.poly.containsLocation(
          new google.maps.LatLng(
            hiddenLatInput.value * 1,
            hiddenLngInput.value * 1
          ),
          gMapsPolygons[i].polygon
        )
      ) {
        polygonsContainAddress = true;
        if (
          localStorage.getItem("addressIsSelected") === "1" &&
          localStorage.getItem("store") * 1 !== gMapsPolygons[i].store * 1
        ) {
          let local = "";
          ...
          if (isConfirmed.value) {
            ...
            if (cartIsDeleted) {
              saveDataToStorage(
                "1",
                gMapsPolygons[i].store,
                storeSelectorInput.value,
                shippingTypeInput.value,
                hiddenLatInput.value,
                hiddenLngInput.value,
                gMapsPolygons[i].deliveryZoneId
              );
...
function saveDataToStorage(
  ...
  Promise.all([
    setMetodoDenvio(localStorage.getItem("shippingType")),
    setDeliveryZoneId(localStorage.getItem("deliveryZoneId")),
  ])
    .then((value) => value.map((value1) => value1.text()))
    .then();
}

function getCoordinates() {
  return fetch("script/delivery/getDeliveryZones.php")
    .then((value) => {
      return value.json();
    })
...
function openAddressSelectorModal() {
  let addressIsSelected = localStorage.getItem("addressIsSelected");
  $("#modalStoreSelectorLocal").modal("show");
  if (!addressIsSelected) {
...
function setMetodoDenvio(metodo) {
  ...
  return fetch("./utils/cambiarMetodoDeEnvio.php", {
    method: "POST",
    body: data,
  });
}
...
function ValidateModalBody(event) {
  ...
  if (
    isNull(localStorage.getItem("address")).length > 2 &&
